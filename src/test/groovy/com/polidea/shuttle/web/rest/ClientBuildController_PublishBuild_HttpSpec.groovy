...
class ClientBuildController_PublishBuild_HttpSpec extends HttpIntegrationSpecification {
    ...
    def "Build is not published by default"() {
        when:
        def buildsResponse = get("/projects/${project.id}/apps/${platform}/${appId}/builds", publisherToken)
    ...
    def "Viewer of not published builds can see not published build"() {
        when:
        def buildsResponse = get("/projects/${project.id}/apps/${platform}/${appId}/builds", viewerToken)
        assert buildsResponse.code() == 200
    ...
    def "Viewer of not published builds cannot publish build"() {
        when:
        def publishResponse = post("/projects/${project.id}/apps/${platform}/${appId}/builds/${buildIdentifier}/publish", viewerToken)
    ...
    def "publish Build"() {
        when:
        def publishResponse = post("/projects/${project.id}/apps/${platform}/${appId}/builds/${buildIdentifier}/publish", publisherToken)
    ...
    def "cannot publish Build if Project does not exist"() {
        ...
        when:
        def publishResponse = post("/projects/${idOfNonExistentProject}/apps/${platform}/${appId}/builds/${buildIdentifier}/publish", publisherToken)
    ...
    def "cannot publish Build if App does not exist"() {
        ...
        when:
        def publishResponse = post("/projects/${project.id}/apps/${platform}/${idOfNonExistentApp}/builds/${buildIdentifier}/publish", publisherToken)
    ...
    def "cannot publish Build if Build does not exist"() {
        ...
        when:
        def publishResponse = post("/projects/${project.id}/apps/${platform}/${appId}/builds/${identifierOfNonExistentBuild}/publish", publisherToken)
    ...
    def "cannot publish Build if App exists but in another project"() {
        ...
        when:
        def publishResponse = post("/projects/${anotherProject.id}/apps/${platform}/${appId}/builds/${buildIdentifier}/publish", publisherToken)
    ...
    def "User cannot publish Build if has no Global Permission to do so (globalPermission: #globalPermission)"(PermissionType globalPermission) {
        ...
        when:
        def publishResponse = post("/projects/${project.id}/apps/${platform}/${appId}/builds/${buildIdentifier}/publish", nonPublisherToken)
