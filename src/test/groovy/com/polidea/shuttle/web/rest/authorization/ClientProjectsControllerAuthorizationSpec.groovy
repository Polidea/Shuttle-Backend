...
class ClientProjectsControllerAuthorizationSpec extends MockMvcIntegrationSpecification {

    @Unroll
    def "should return status code #statusCode.name() for token #tokenValue when mute project"(String tokenValue, HttpStatus statusCode) {
        when:
        def result = post("/projects/$TEST_PROJECT_ID/mute", tokenValue)
    ...
    def "should return status code #statusCode.name() for token #tokenValue when unmute project"(String tokenValue, HttpStatus statusCode) {
        when:
        def result = delete("/projects/$TEST_PROJECT_ID/mute", tokenValue)
